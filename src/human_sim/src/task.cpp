...
void Plan::show()
{
	...
	for(int i=0; i<action_series_.size(); i++)
		printf("\ttype=%s pose= (%f, %f, %f) state=%d\n", action_series_[i].action.type.c_str(), action_series_[i].action.destination.x, action_series_[i].action.destination.y, action_series_[i].action.destination.theta, action_series_[i].state);
}
