...
int main(int argc, char** argv)
{
	...
	ros::Subscriber sub_human_other_pose = nh.subscribe("/morse/human2_pose", 100, humanOtherPoseCallback);
	pub_sim_human_other_pose = nh.advertise<geometry_msgs::Pose2D>("in/human_other_pose", 100);

	ros::Subscriber sub_human_other_vel = nh.subscribe("/morse/human2_vel", 100, humanOtherVelCallback);
